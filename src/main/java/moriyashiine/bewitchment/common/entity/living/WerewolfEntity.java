...
public class WerewolfEntity extends BWHostileEntity {
	...
	public void tick() {
		...
		if (!getWorld().isClient) {
			...
			if (storedVillager != null && age % 20 == 0 && (getWorld().isDay() || BewitchmentAPI.getMoonPhase(getWorld()) != 0)) {
				...
				EntityVillagerMCA entity = EntityType.MCA.create(getWorld());
				if (entity != null) {
	...
	public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, @Nullable EntityData entityData, @Nullable NbtCompound entityTag) {
        ...
        // Aqui deve haver lógica para evitar criar mais de uma vez
        NbtCompound villagerNbt = EntityType.VILLAGER.create(world).writeNbt(new NbtCompound());
        if (villagerNbt != null) {
