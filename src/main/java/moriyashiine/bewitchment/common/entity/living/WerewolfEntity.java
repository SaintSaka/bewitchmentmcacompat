...
public class WerewolfEntity extends BWHostileEntity {
    ...
    public static DefaultAttributeContainer.Builder createAttributes() {
        return MobEntity.createMobAttributes()
            ...
            .add(ReachEntityAttributes.ATTACK_RANGE, 1)
            .add(StepHeightEntityAttributeMain.STEP_HEIGHT, 1);
    }
    ...
    public static boolean canSpawn(EntityType<WerewolfEntity> type, ServerWorldAccess world, SpawnReason spawnReason, BlockPos pos, Random random) {
        return world.toServerWorld().isNight() && BewitchmentAPI.getMoonPhase(world.toServerWorld().toServerWorld()) == 0 && 
    ...
    public void tick() {
        ...
        if (!getWorld().isClient) {
            ...
            if (storedVillager != null && age % 20 == 0 && 
                (getWorld().isDay() || BewitchmentAPI.getMoonPhase(getWorld()) != 0)) {
                
                VillagerEntity entity = EntityType.VILLAGER.create(getWorld());
                if (entity != null) {
                    PlayerLookup.tracking(this).forEach(trackingPlayer -> 
                        SpawnSmokeParticlesPacket.send(trackingPlayer, this));
                    
                    getWorld().playSound(null, getX(), getY(), getZ(), 
                        BWSoundEvents.ENTITY_GENERIC_TRANSFORM, getSoundCategory(), getSoundVolume(), getSoundPitch());
                    ...
                    BWComponents.CURSES_COMPONENT.maybeGet(entity).ifPresent(entityCursesComponent -> 
                        BWComponents.CURSES_COMPONENT.maybeGet(this).ifPresent(thisCursesComponent -> {
    ...
    public static WerewolfEntity transformVillagerToWerewolf(EntityVillagerMCA villager, World world) {
        WerewolfEntity werewolf = new WerewolfEntity(EntityType.WEREWOLF, world);
        
        // Armazenar dados do villager
        werewolf.storedVillager = villager.writeNbt(new NbtCompound());
        
        // Copiar atributos ou status do villager para o werewolf
        werewolf.setHealth(villager.getHealth());
        werewolf.setPosition(villager.getX(), villager.getY(), villager.getZ());
        
        return werewolf;
    }

    // Método para verificar se deve transformar
    ...
    public EntityData initialize(ServerWorldAccess world, LocalDifficulty difficulty, SpawnReason spawnReason, @Nullable EntityData entityData, @Nullable NbtCompound entityTag) {
        ...
        if (spawnReason == SpawnReason.NATURAL || spawnReason == SpawnReason.SPAWN_EGG) {
            // Aqui você pode usar a EntityVillagerMCA
    ...
    protected void initGoals() {
        ...
        targetSelector.add(1, new ActiveTargetGoal<>(this, LivingEntity.class, 10, true, false, 
            entity -> entity instanceof PlayerEntity || entity instanceof SheepEntity || entity instanceof MerchantEntity || 
            entity.getGroup() == EntityGroup.ILLAGER));
    }
